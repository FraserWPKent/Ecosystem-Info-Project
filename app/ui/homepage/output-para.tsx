...
export default async function OutBlock({data, species}: Props){
    ...
    if(status !== "Failure" && imageData.summary !== ""){
        return (
            <>
                <div className={`border-1 p-4 my-3 rounded-lg border-[#293734] bg-[var(--main-element)] drop-shadow-xl overflow-hidden 
                    w-full h-full ${imageData.summary==="" ? '' : ''} `} 
                    ...
                    <div className="w-full pl-[1rem] mt-1/2" 
                    // style={{width:"100%", paddingLeft:"1rem"}}
                    >
                        <div className={`w-full h-full align-center text-center ${imageData.summary==="" ? '' : ''}`}>
                            <p className={`font-bold break-normal text-white w-full h-full flex-1`}>{data.targetName}</p>
                            <p className={`font-bold break-normal text-white w-full h-full mb-4 flex-1`}>{status}</p>
                        </div>
        ...
        return(
            <>
                <div className={`border-1 p-4 my-3 rounded-lg border-[#293734] bg-[var(--main-element)] drop-shadow-xl overflow-hidden 
                    ...
                    // style={{width:"100%", paddingLeft:"1rem"}}
                    >
                        <div className={`w-full h-full align-center text-center text-lg ${imageData.summary==="" ? '' : ''}`}>
                            <p className={`font-bold break-normal text-white w-full h-full flex-1`}>{data.targetName}</p>
                            <p className={`font-bold break-normal text-white w-full h-full flex-1`}>{status}</p>
                        </div>
                    {/* <Button id = {data.id} species = {species}/> */}
                        <p className = "break-normal text-white w-full text-left whitespace-pre-line">
                            {imageData.summary}
                    ...
                    <div className="sm:w-full lg:w-[200%]"
                    // style={{width:"100%"}}
                    >
                        <OutputImage 
                        primaryName={data.targetName} 
